...
Note on **buff-on-join**: members spawn pre-buffed with a *base* kit (`PREBUFF_*` — base Might/Empower/etc.), but the party buffer maintains its *full* known kit (Greater Might/Greater Empower, Berserker, Bless the Body, …). On join the buffer tops up the buffs the prebuff didn't include — so it looks like "buffers buff them the moment they join". It's benign (happens out of combat, exactly when buffing should) but redundant; aligning `PREBUFF_*` to the maintained kit would remove the on-join top-up if desired.
